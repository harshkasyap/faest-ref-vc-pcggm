...
int ChalDec(const uint8_t* chal, unsigned int i, unsigned int k0, unsigned int t0, unsigned int k1,
  ...
  //printf("lo %zu, hi %zu", lo, hi);
  for (unsigned int j = lo; j < hi + 4; ++j) {
...
void vole_commit(const uint8_t* rootKey, const uint8_t* iv, unsigned int ellhat,
  ...
  //uint8_t* ui = malloc(tau * ellhat_bytes);
  uint8_t* ui = malloc(ellhat_bytes);

  // Step 1
  //uint8_t* expanded_keys = malloc(tau * lambda_bytes);
  //prg(rootKey, iv, expanded_keys, lambda, lambda_bytes * tau);

  uint8_t* expanded_keys = malloc(lambda_bytes);
  prg(rootKey, iv, expanded_keys, lambda, lambda_bytes);
  ...
  unsigned int i = 0;
    // Step 4
    //unsigned int depth = i < tau0 ? k0 : k1;
    unsigned int depth = k0 + 4;

    // Step 5
    vector_commitment(expanded_keys + lambda_bytes, iv, params, lambda, vecCom, depth);
    // Step 6
    ConvertToVole(iv, vecCom->sd, false, lambda, depth, ellhat_bytes, ui + i * ellhat_bytes,
                  v[v_idx]);
    // Step 7 (and parts of 8)
    v_idx += depth;
    // Step 12 (part)
    H1_update(&h1_ctx, vecCom->h, lambda_bytes * 2);
  //}
  free(expanded_keys);
  // Step 9
  memcpy(u, ui, ellhat_bytes);
  //for (unsigned int i = 1; i < tau; i++) {
    // Step 11
    xor_u8_array(u, ui + ellhat_bytes, c + ellhat_bytes, ellhat_bytes);
  //}
  free(ui);
...
void vole_reconstruct(const uint8_t* iv, const uint8_t* chall, const uint8_t* const* pdec,
  ...
  //uint8_t* sd = malloc((1 << MAX(k0, k1)) * lambda_bytes);
  uint8_t* sd = malloc((1 << (k0 + 4)) * lambda_bytes);
  memset(sd, 0, lambda_bytes);
  ...
  vecComRec.k   = calloc(getBinaryTreeNodeCount(k0 + 4), lambda_bytes);
  vecComRec.com = malloc((1 << (k0 + 4)) * lambda_bytes * 2);
  vecComRec.s   = malloc((1 << (k0 + 4)) * lambda_bytes);
  ...
  unsigned int i = 0;
    ...
    // Step 3
    uint8_t chalout[MAX_DEPTH + 4];
    ChalDec(chall, i, k0 + 4, tau0, k1, tau1, chalout);
    // Step 4
    unsigned int idx = NumRec(depth, chalout);

    // Step 5
    
    size_t length = sizeof(chalout) / sizeof(chalout[0]);
    //printf(" \n Hello20 A %zu B %zu length %zu ", depth, MAX_DEPTH, length);
    //printf("chalout %zu", chalout);
    vector_reconstruction(iv, pdec[i], com_j[i], chalout, lambda, depth, &vecComRec);
